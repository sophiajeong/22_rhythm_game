...
pygame.mixer.music.load("Dua Lipa - Levitating.mp3")
pygame.mixer.music.play(0)

#======== 함수 ===============================
#키 이벤트 처리하기
def resultProcess(direction):
    global isColl, score, DrawResult, result_ticks, level
...
def eventProcess(): # 눌려진 키의 이벤트를 가져와서
    global isActive, score, start_time
...
class Direction(object):
    ...
    def draw(self):
        if self.y >= SCREEN_HEIGHT:
            ...
            self.y += -1 +i   # 위의 경우가 전부 아닌 경우 y가 1씩 증가하며 이미지가 그려진다.
...
#문자 넣기
def setText():
    global score, total_time, left_time, level
    remain_time = (pygame.time.get_ticks() - start_time) / 1000
